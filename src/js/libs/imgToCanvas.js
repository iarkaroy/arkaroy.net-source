...
export async function imgToCanvas(src, owidth, oheight, isGrey = false) {
    return new Promise((resolve, reject) => {
        ...
        loadImage(src).then(img => {
            const canvas = image2canvas(img, owidth, oheight, isGrey);
            resolve(canvas);
        }, reject);
    });
};

export function image2canvas(image, owidth, oheight, isGrey = false) {
    const ctx = document.createElement('canvas').getContext('2d');
    ctx.canvas.width = owidth;
    ctx.canvas.height = oheight;
    const { width, height, offsetX, offsetY } = cover(ctx.canvas.width, ctx.canvas.height, image.width, image.height);
